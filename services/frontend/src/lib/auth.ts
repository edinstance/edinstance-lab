      ...
      before: [
        {
          ...
          handler: createAuthMiddleware((ctx) => {
            ...
            if (
              isBlocked(failedSignupPasswords, ip, maxFailedSignupPasswords)
            ) {
              throw APIError.from("TOO_MANY_REQUESTS", {
                code: "TOO_MANY_SIGN_UP_PASSWORD_ATTEMPTS",
                message: "Too many sign-up password attempts",
              });
            }
            ...
            if (!isEqualConstantTime(password, getAdminPassword())) {
              incrementFailure(
                failedSignupPasswords,
                ip,
                signupPasswordWindowMs,
              );
              throw APIError.from("FORBIDDEN", {
                code: "INVALID_SIGN_UP_PASSWORD",
                message: "Invalid sign-up password",
              });
            }
