...
const fieldClass =
  "grid gap-2 [&>span]:font-mono [&>span]:text-[.72rem] [&>span]:font-black [&>span]:uppercase [&>span]:leading-none [&>input]:min-h-[42px] [&>input]:border [&>input]:border-[#9c927f] [&>input]:bg-[#fffdf7] [&>input]:px-3 [&>input]:font-mono [&>input]:text-base [&>input]:font-black [&>input]:text-[#17211b]";
const buttonClass =
  "min-h-[30px] border border-[#9c927f] bg-[#fffdf7e0] px-2.5 font-mono text-[.72rem] font-extrabold leading-none text-[#17211b] transition-colors hover:border-[#17211b] hover:bg-[#17211b] hover:text-[#fffdf7] disabled:cursor-not-allowed disabled:opacity-55";
const rowClass =
...
function ManagePage() {
  ...
  const [databaseForm, setDatabaseForm] = useState({
    ...
    poolerEnabled: true,
    poolerInstances: "2",
    poolMode: "session" as "session" | "transaction",
    public: false,
    publicHostname: "",
    ...
    try {
      const database = await createDatabase({
        ...
        publicSourceCidrs: databaseForm.restrictPublicSources
          ? databaseForm.publicSourceCidrs.split(",").map((cidr) => cidr.trim()).filter(Boolean)
          : [],
      });
  ...
  return (
    <main className="min-h-screen p-7">
      <header className="mx-auto mb-6 flex max-w-[1180px] items-end justify-between max-[860px]:grid max-[860px]:items-start max-[860px]:gap-4">
        <div>
          <p className="mb-2 mt-0 font-mono text-[.68rem] font-black uppercase leading-none text-[#66736b]">
            platform control
          </p>
          <h1 className="m-0 text-[1.8rem] leading-none">Services</h1>
        </div>
        <Link
          className={`${buttonClass} inline-flex items-center py-2.5 uppercase no-underline`}
          to="/"
        >
          Service graph
        </Link>
      ...
      <section
        ...
        aria-label="Create platform app"
      >
        <form
          className="grid grid-cols-[1fr_minmax(260px,2fr)_120px_120px] gap-3.5 p-4 max-[860px]:grid-cols-1"
        ...
        >
          ...
          <label className={`${fieldClass} col-[1/-2] max-[860px]:col-auto`}>
            ...
            <input
              ...
              value={form.domains}
              onChange={updateForm}
              placeholder="app.local.edinstance.uk, app.edinstance.uk"
            />
          </label>
          <button
            className={`${buttonClass} min-h-[42px] self-end`}
            disabled={saving}
            type="submit"
          >
        ...
        {notice ? (
          <p className="m-0 border-t border-[#c9c1afbf] px-4 py-[18px] font-mono text-[.82rem] font-extrabold leading-[1.4] text-[#517a38]">
            {notice}
          </p>
        ) : null}
      </section>
      ...
      >
        ...
        >
          <label className={fieldClass}>
            <span>Name / DNS prefix</span>
            <input
              required
              pattern="[a-z0-9]([-a-z0-9]*[a-z0-9])?"
              value={databaseForm.name}
              onChange={(e) =>
                setDatabaseForm({ ...databaseForm, name: e.target.value })
              }
            />
          </label>
          <label className={fieldClass}>
            ...
            <input
              ...
              onChange={(e) =>
                setDatabaseForm({ ...databaseForm, database: e.target.value })
              }
            />
          </label>
          <label className={fieldClass}>
            ...
            <input
              ...
              onChange={(e) =>
                setDatabaseForm({ ...databaseForm, owner: e.target.value })
              }
            />
          </label>
          <label className={fieldClass}>
            ...
            <input
              ...
              onChange={(e) =>
                setDatabaseForm({ ...databaseForm, password: e.target.value })
              }
            />
          </label>
          <label className={fieldClass}>
            ...
            <select
              className="min-h-[42px] border border-[#9c927f] bg-[#fffdf7] px-3 font-mono font-black"
              value={databaseForm.version}
              onChange={(e) =>
                setDatabaseForm({ ...databaseForm, version: e.target.value })
              }
            >
              <option>17</option>
              <option>16</option>
          ...
          <label className={fieldClass}>
            ...
            <input
              ...
              max="5"
              type="number"
              value={databaseForm.instances}
              onChange={(e) =>
                setDatabaseForm({ ...databaseForm, instances: e.target.value })
              }
            />
          </label>
          <label className={fieldClass}>
            <span>Storage</span>
            <input
              value={databaseForm.storageSize}
              onChange={(e) =>
                setDatabaseForm({
                  ...databaseForm,
          ...
          <label className={fieldClass}>
            ...
            <select
              className="min-h-[42px] border border-[#9c927f] bg-[#fffdf7] px-3 font-mono font-black"
              disabled={!databaseForm.poolerEnabled}
              value={databaseForm.poolMode}
              onChange={(e) =>
                setDatabaseForm({
                  ...databaseForm,
                  poolMode: e.target.value as "session" | "transaction",
                })
              }
            >
          ...
          <label className="flex items-center gap-2 font-mono text-sm font-black">
            <input
              type="checkbox"
              checked={databaseForm.poolerEnabled}
              disabled={databaseForm.public}
              onChange={(e) =>
                setDatabaseForm({
                  ...databaseForm,
                  poolerEnabled: e.target.checked,
                })
              }
            />{" "}
            Enable PgBouncer
          ...
          <label className={fieldClass}>
            <span>Pooler replicas</span>
            <input
              min="1"
              max="5"
              type="number"
              disabled={!databaseForm.poolerEnabled}
              value={databaseForm.poolerInstances}
              onChange={(e) =>
                setDatabaseForm({
                  ...databaseForm,
                  poolerInstances: e.target.value,
                })
              }
            />
          </label>
          <label className="flex items-center gap-2 font-mono text-sm font-black">
            <input
              type="checkbox"
              checked={databaseForm.public}
              onChange={(e) =>
                setDatabaseForm({
                  ...databaseForm,
                  public: e.target.checked,
                  poolerEnabled: e.target.checked ? true : databaseForm.poolerEnabled,
                })
              }
            />{" "}
            Public through PgBouncer
          </label>
          <label className={fieldClass}>
            <span>Public hostname</span>
            <input
              required={databaseForm.public}
              disabled={!databaseForm.public}
              placeholder="db.edinstance.uk"
              value={databaseForm.publicHostname}
              onChange={(e) =>
                setDatabaseForm({
                  ...databaseForm,
                  publicHostname: e.target.value,
                })
              }
            />
          </label>
          <label className="flex items-center gap-2 font-mono text-sm font-black">
            <input type="checkbox" checked={databaseForm.restrictPublicSources} disabled={!databaseForm.public} onChange={(e) => setDatabaseForm({ ...databaseForm, restrictPublicSources: e.target.checked })} />{" "}
            Advanced source restrictions
          </label>
          <label className={`${fieldClass} col-span-2 max-[860px]:col-auto`}>
            <span>Allowed source CIDRs (optional)</span>
            <input
              required={databaseForm.public && databaseForm.restrictPublicSources}
              disabled={!databaseForm.public || !databaseForm.restrictPublicSources}
              placeholder="203.0.113.10/32, 198.51.100.0/24"
              value={databaseForm.publicSourceCidrs}
              onChange={(e) =>
                setDatabaseForm({
                  ...databaseForm,
                  publicSourceCidrs: e.target.value,
                })
              }
            />
          </label>
        ...
        {databaseForm.public ? (
          <p className="m-0 border-t border-[#c9c1af] px-4 py-3 font-mono text-xs font-bold text-[#b0822e]">
            Creates a dedicated MetalLB address on port 5432. Point DNS to your
            WAN IP and forward TCP 5432 to the assigned MetalLB IP. Public
            traffic always terminates at PgBouncer; cluster services use the
            internal PgBouncer hostname.
          </p>
        ) : null}
        {databases.length > 0 ? (
              ...
              >
                ...
                {database.public ? (
                  <code className="text-[#d65236]">
                    ...
                    {database.publicSourceCidrs?.length ? database.publicSourceCidrs.join(", ") : "all sources"}
                  </code>
                ) : null}
                <span>
      ...
      >
        <div
          className={`${rowClass} min-h-0 bg-[#17211b0f] font-mono text-[.68rem] font-black uppercase leading-none text-[#66736b] max-[860px]:hidden`}
        >
          <span>Name</span>
          <span>Status</span>
          <span>Image</span>
        ...
        {loading ? (
          <p className="m-0 px-4 py-[18px] font-mono text-[.82rem] font-extrabold leading-[1.4]">
            Loading services...
          </p>
        ) : null}
        {error ? (
          <p className="m-0 px-4 py-[18px] font-mono text-[.82rem] font-extrabold leading-[1.4] text-[#d65236]">
            {error}
          </p>
        ) : null}
        {!loading && !error && apps.length === 0 ? (
          <p className="m-0 px-4 py-[18px] font-mono text-[.82rem] font-extrabold leading-[1.4]">
            No platform services yet.
          </p>
        ) : null}
        {apps.map((app) => (
          <article className="border-t border-[#c9c1afbf]" key={app.name}>
            ...
            >
              <strong className="font-mono text-[.9rem] font-black leading-[1.3]">
                {app.name}
              </strong>
              <span className="flex content-start flex-wrap gap-2">
                <span
                  className={`border px-[7px] py-[5px] font-mono text-[.62rem] font-black uppercase leading-none ${app.ready ? "border-[#517a3861] text-[#517a38]" : "border-[#b0822e75] text-[#b0822e]"}`}
                >
                  {app.status}
                </span>
              ...
              <code className="font-mono text-[.76rem] font-extrabold leading-[1.45] text-[#2d6f8f]">
                {app.image}
              </code>
              <span>
              ...
              <span className="flex flex-wrap items-start gap-2">
                <label className="inline-flex min-w-0 cursor-pointer">
                  <input
                    ...
                    type="file"
                    onChange={(event) => void handleEnvFile(app, event)}
                  />
                  <span className={`${buttonClass} inline-flex items-center`}>
                    {busyApp === app.name ? "Working..." : "Upload env"}
                  </span>
                </label>
                <button
                  ...
                  type="button"
                  onClick={() => void toggleEnvironment(app)}
                >
            ...
            {envApp === app.name ? (
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className="m-0 font-mono text-[.68rem] font-black uppercase text-[#66736b]">
                    ...
                    >
                      ...
                      <span className="flex gap-2">
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            setEnvDraft({ name: variable.name, value: "" })
                          }
                        >
                          Replace
                        </button>
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            void handleDeleteEnvVar(app, variable.name)
                          }
                        >
                          Delete
                        </button>
                ...
                <form
                  ...
                  onSubmit={(event) => void handleSetEnvVar(app, event)}
                >
                  <label className={fieldClass}>
                    <span>Variable</span>
                    <input
                  ...
                  <label className={fieldClass}>
                    <span>
                      Value{" "}
                      {envDraft.name &&
                      (envVars[app.name] ?? []).some(
                        (item) => item.name === envDraft.name
                      )
                        ? "(replaces existing)"
                        : ""}
                    </span>
                    <input
                      ...
                      onChange={(event) =>
                        setEnvDraft({ ...envDraft, value: event.target.value })
                      }
                    />
                  </label>
                  <button
                ...
                <p className="m-0 font-mono text-xs text-[#66736b]">
                  Values are encrypted at rest and are never returned by the
                  API. Saving or deleting requests a workload rollout.
                </p>
              </section>
            ) : null}
          </article>
